...
// initially copied from SimParser2.cpp
...
static const char* versionName(SimulaVersion v) {
    switch( v ) {
    case Sim70:
        return "67";
    case Sim75:
...
bool Parser3::versionCheck(SimulaVersion minVersion, const char* feature) {
    if( mdl->getVersion() >= minVersion )
        return true;
    if( feature )
        error(la, QString("'%1' requires Simula %2 or later").arg(feature).arg(versionName(minVersion)));
    return false;
}

bool Parser3::versionMax(SimulaVersion maxVersion, const char* feature) {
    if( mdl->getVersion() <= maxVersion )
        return true;
    if( feature )
        error(la, QString("'%1' is not supported in Simula %2").arg(feature).
              arg(versionName(mdl->getVersion())));
    return false;
}
...
DeclList Parser3::virtual_spec() {
    ...
    if( FIRST_procedure_specification(la.d_type) ) {
        if( versionCheck(Sim86, "procedure specification") ) {
            Token id;
            Type* sig = procedure_specification(id);
            if( isProcedure && res.size() == 1 )
            {
                if( id.d_type == Tok_identifier && id.d_id != res.first()->sym )
                    error(id, "the procedure specification must repeat the virtual procedure identifier");
                res.first()->setType(sig);
            }else
            {
                error(id, "a procedure specification requires a single virtual procedure identifier");
                delete sig;
            }
        }
    }
...
Declaration* Parser3::procedure_heading(bool detached) {
    expect(Tok_identifier, false, "procedure_heading");
    Declaration* procDecl;
    if( detached )
    {
        procDecl = new Declaration(Declaration::Procedure);
        procDecl->name = cur.d_val;
        procDecl->sym = cur.d_id;
    }else
        procDecl = mdl->addDecl(cur.d_id, cur.d_val, Declaration::Procedure);
...
Type* Parser3::procedure_specification(Token& id) {
    expect(Tok_IS, false, "procedure_specification");

    Type* resultType = 0;
    if( FIRST_type(la.d_type) )
        resultType = type();
    expect(Tok_PROCEDURE, false, "procedure_specification");

    id = la;
    Declaration* heading = procedure_heading(true);

    if( la.d_type != Tok_Semi && la.d_type != Tok_END )
    {
        error(la, "the procedure body of a procedure specification must be empty");
        Statement* body = procedure_body();
        if( body )
            Statement::deleteAll(body);
    }

    return toProcType(resultType, heading);
}

Type* Parser3::toProcType(Type* resultType, Declaration* heading)
{
    Type* res = new Type(Type::Procedure);
    res->setType(resultType);

    Declaration* param = heading->link;
    heading->link = 0;
    while( param )
    {
        Declaration* next = param->next;
        param->next = 0;
        param->outer = 0;
        res->subs << param;
        param = next;
    }
    Declaration::deleteAll(heading);
...
void Parser3::external_declaration() {
    ...
    if( la.d_type == Tok_identifier || FIRST_type(la.d_type) || la.d_type == Tok_PROCEDURE ) {
        ...
        if( FIRST_procedure_specification(la.d_type) &&
                versionCheck(Sim86, "procedure specification") )
        {
            Token id;
            Type* sig = procedure_specification(id);
            Declaration* proc = list.size() == 1 ? list.first() : 0;
            if( proc && t == 0 && kind != 0 )
            {
                proc->setType(sig);
                if( id.d_type == Tok_identifier )
                {
                    // the specification declares the local identifier, external_item the external one
                    proc->sym = id.d_id;
                    proc->name = id.d_val;
                }
            }else
            {
                error(id, "a procedure specification requires an external kind and a single, untyped external item");
                delete sig;
            }
        }else
...
void Parser3::array_segment(Type* elemType) {
    ...
    // array_segment ::= identifier { ',' identifier } bounds
